...
public class TusharePro {
  ...
  public static class Builder {
    ...
    private static final OkHttpClient DEFAULT_HTTP_CLIENT = new OkHttpClient.Builder()
        ...
        .connectTimeout(42, TimeUnit.SECONDS).writeTimeout(42, TimeUnit.SECONDS).readTimeout(42, TimeUnit.SECONDS)
        .build();
    ...
    private Function<byte[], byte[]> httpFunction = requestBytes -> { // requestBytes -> function -> responseBytes
      try {
        okhttp3.Request request =
            new okhttp3.Request.Builder().url(API_URL).post(RequestBody.create(requestBytes, MEDIA_TYPE)).build();
    ...
    public Builder copy() {
      return new Builder().setToken(token).setMaxRetries(maxRetries).setRetrySleepTimeUnit(retrySleepTimeUnit)
          .setRetrySleepTimeOut(retrySleepTimeOut).setRequestExecutor(requestExecutor).setHttpFunction(httpFunction);
    }
    ...
    // Arrays.stream(functions).forEach(function -> defaultMapperMap.put(function.toString(),
    // function));
    // return this;
    // }
  }
  ...
  public TusharePro(String token, Double integral, int maxRetries, TimeUnit retrySleepTimeUnit, long retrySleepTimeOut,
      ExecutorService requestExecutor, Function<byte[], byte[]> httpFunction) {
    this.token = token;
    this.integral = integral;
    this.maxRetries = maxRetries;
    this.retrySleepTimeUnit = retrySleepTimeUnit;
