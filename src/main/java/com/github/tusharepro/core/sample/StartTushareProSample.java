...
public class StartTushareProSample {
  ...
  private static void start() throws IOException {
    ...
    final TusharePro.Builder builder = new TusharePro.Builder().setToken("**********"); // 你的token
    ...
    // 一个完整的例子
    TushareProService.stockBasic(new Request<StockBasicEntity>(builder.copy() // 将配置拷贝
        ...
        .setHttpFunction(requestBytes -> { // requestBytes -> function -> responseBytes
          try {
            okhttp3.Request request = new okhttp3.Request.Builder().url("http://api.tushare.pro")
                .post(RequestBody.create(requestBytes, MediaType.parse("application/json; charset=utf-8"))).build();
          ...
          return null;
        }).build()) {}.allFields().param(StockBasic.Params.exchange.value("SZSE")).param(list_status)).stream()
        .filter(x -> "银行".equals(x.getIndustry())).forEach(System.out::println);
  }
