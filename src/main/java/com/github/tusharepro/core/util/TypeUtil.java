...
public final class TypeUtil {
   ...
   */
  public static void findClasses(Class<?> targetSupperClass, Class<?> currentClass, List<Class<?>> classList,
      List<Class<?>> targetClassList) {
    classList.add(currentClass);
    // 寻找接口
    for (Class<?> interfaceClass : currentClass.getInterfaces()) {
      if (interfaceClass.equals(targetSupperClass))
        targetClassList.add(currentClass);
