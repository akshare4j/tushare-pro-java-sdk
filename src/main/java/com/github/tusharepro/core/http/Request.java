...
public abstract class Request<T extends BaseBean> {
  ...
  public Request<T> allFields() {
    return fields(Arrays.stream(Arrays.stream(beanInterfaceClass.getClasses())
        .filter(x -> x.getSimpleName().equals("Fields")).findAny().orElse(Object.class).getFields()).map(x -> {
